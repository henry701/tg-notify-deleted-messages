    ...
    if additional:
        ...
        try:
            ...
            if "Event loop stopped before Future completed" not in str(e):
                logger.error(
                    "[exit] Error while running user-provided cleanupper", exc_info=True
                )
        else:
...
def add_signal_handlers(loop, closer):
    for signame in {"SIGINT", "SIGTERM"}:
        loop.add_signal_handler(
            getattr(signal, signame), functools.partial(ask_exit, signame, loop, closer)
        )
...
def create_app_and_start_jobs() -> Tuple[flask.Flask, Callable[[], None]]:
    ...
    def worker_function(
        loop: asyncio.AbstractEventLoop, sync_closer: Callable[[], Any]
    ):
